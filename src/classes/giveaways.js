...
class giveaways {
	...
	static async startTimer(message, data, instant = false) {
		...
		setTimeout(async () => {
			...
			if (!winners) {
				message.channel.send('𝐍𝐨𝐭 𝐞𝐧𝐨𝐮𝐠𝐡 𝐩𝐞𝐨𝐩𝐥𝐞 𝐩𝐚𝐫𝐭𝐢𝐜𝐢𝐩𝐚𝐭𝐞𝐝 𝐢𝐧 𝐭𝐡𝐢𝐬 𝐠𝐢𝐯𝐞𝐚𝐰𝐚𝐲');
				data.ended = true;
				data.save();
				const embed = msg.embeds[0];
				embed.description = `🎁 𝐏𝐫𝐢𝐳𝐞: **${data.prize}**\n🎊 𝐇𝐨𝐬𝐭𝐞𝐝 𝐛𝐲: <@${data.host.toString()}>\n⏲️ 𝐖𝐢𝐧𝐧𝐞𝐫(𝐬): 𝐧𝐨𝐧𝐞`;
			...
			const dmEmbed = new Discord.MessageEmbed()
				.setTitle('𝐘𝐨𝐮 𝐰𝐨𝐧! ╭∩╮(ಠ_ಠ)╭∩╮')
				.setDescription(`𝐘𝐨𝐮 𝐡𝐚𝐯𝐞 𝐰𝐨𝐧 𝐭𝐡𝐞 𝐠𝐢𝐯𝐞𝐚𝐰𝐚𝐲 𝐢𝐧 **${msg.guild.name}**!\n𝐏𝐫𝐢𝐳𝐞: [${data.prize}](https://discord.com/${msg.guild.id}/${msg.channel.id}/${data.messageID})`)
				.setColor('RANDOM')
				.setFooter('𝐂𝐨𝐧𝐠𝐫𝐚𝐭𝐮𝐥𝐚𝐭𝐢𝐨𝐧𝐬!');
			winners.forEach((user) => {
				message.guild.members.cache.get(user).send(dmEmbed);
			});
			const embed = msg.embeds[0];
			embed.description = `🎁 𝐏𝐫𝐢𝐳𝐞: **${data.prize}**\n🎊 𝐇𝐨𝐬𝐭𝐞𝐝 𝐛𝐲: <@${data.host.toString()}>\n⏲️ 𝐖𝐢𝐧𝐧𝐞𝐫(𝐬): ${winners.map(winner => `<@${winner}>`).join(', ')}`;
			msg.edit('', { embed: embed });
			data.ended = true;
			data.save();
			utils.editButtons(message.client, data);
	...
	static async endByButton(client, messageID, button) {
		...
		if (res == 'ENDED') button.reply.send('𝐓𝐡𝐞 𝐠𝐢𝐯𝐞𝐚𝐰𝐚𝐲 𝐡𝐚𝐬 𝐚𝐥𝐫𝐞𝐚𝐝𝐲 𝐞𝐧𝐝𝐞𝐝', { ephemeral: true });
	}
	...
	static async end(message, data, giveawaymsg) {
		...
		message.channel.send(`${winners.map(winner => `<@${winner}>`).join(', ')} 𝐘𝐨𝐮 𝐰𝐨𝐧 ${data.prize} 𝐂𝐨𝐧𝐠𝐫𝐚𝐭𝐮𝐥𝐚𝐭𝐢𝐨𝐧𝐬! 𝐇𝐨𝐬𝐭𝐞𝐝 𝐛𝐲 ${message.guild.members.cache.get(data.host).toString()}`, { component: await this.gotoGiveaway(data) });
		const dmEmbed = new Discord.MessageEmbed()
			.setTitle('𝐘𝐨𝐮 𝐰𝐨𝐧! ╭∩╮(ಠ_ಠ)╭∩╮')
			.setDescription(`𝐘𝐨𝐮 𝐡𝐚𝐯𝐞 𝐰𝐨𝐧 𝐭𝐡𝐞 𝐠𝐢𝐯𝐞𝐚𝐰𝐚𝐲 𝐢𝐧 **${giveawaymsg.guild.name}**!\n𝐏𝐫𝐢𝐳𝐞: [${data.prize}](https://discord.com/${giveawaymsg.guild.id}/${giveawaymsg.channel.id}/${data.messageID})`)
			.setColor('RANDOM')
			.setFooter('𝐂𝐨𝐧𝐠𝐫𝐚𝐭𝐮𝐥𝐚𝐭𝐢𝐨𝐧!');
		winners.forEach((user) => {
			message.guild.members.cache.get(user).send(dmEmbed);
		});
		const embed = giveawaymsg.embeds[0];
	...
	static async drop({ message, channel, prize, host }) {
		...
		collector.on('collect', async (b) => {
			b.defer();
			ended = true;
			b.channel.send(`<@${b.clicker.user.id}> 𝐰𝐨𝐧 𝐭𝐡𝐞 𝐝𝐫𝐨𝐩 𝐠𝐢𝐯𝐞𝐚𝐰𝐚𝐲!!`);

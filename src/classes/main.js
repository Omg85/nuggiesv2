	...
	static async buttonclick(client, button) {
		...
		if (id.startsWith('giveaways')) {
			...
			if (tag[1] === 'enter') {
				...
				if (!data.clickers.includes(button.clicker.user.id)) {
					...
					data.save();
					return button.reply.send('𝐘𝐨𝐮 𝐡𝐚𝐯𝐞 𝐞𝐧𝐭𝐞𝐫𝐞𝐝 𝐭𝐡𝐢𝐬 𝐠𝐢𝐯𝐞𝐚𝐰𝐚𝐲 𝐠𝐨𝐨𝐝𝐥𝐮𝐜𝐤 (;´༎ຶٹ༎ຶ`)', true);
				}
				if (data.clickers.includes(button.clicker.user.id)) {
					return button.reply.send('𝐘𝐨𝐮 𝐚𝐥𝐫𝐞𝐚𝐝𝐲 𝐞𝐧𝐭𝐞𝐫𝐞𝐝 𝐭𝐡𝐢𝐬 𝐠𝐢𝐯𝐞𝐚𝐰𝐚𝐲 ಠ_ಠ', true);
				}
			}
			if (tag[1] === 'reroll') {
				...
				try {
					...
					console.log(err);
					return button.message.channel.send('𝐔𝐧𝐚𝐛𝐥𝐞 𝐭𝐨 𝐟𝐢𝐧𝐝 𝐭𝐡𝐢𝐬 𝐠𝐢𝐯𝐞𝐚𝐰𝐚𝐲');
				}
				if (!win.length) return button.message.channel.send('𝐓𝐡𝐞𝐫𝐞 𝐚𝐫𝐞 𝐧𝐨𝐭 𝐞𝐧𝐨𝐮𝐠𝐡 𝐩𝐞𝐨𝐩𝐥𝐞 𝐢𝐧 𝐭𝐡𝐢𝐬 𝐠𝐢𝐯𝐞𝐚𝐰𝐚𝐲');
			...
			if (tag[1] === 'end') {
				...
				if (button.clicker.user.id !== tag[2]) return button.reply.send('𝐘𝐨𝐮 𝐜𝐚𝐧𝐧𝐨𝐭 𝐞𝐧𝐝 𝐭𝐡𝐢𝐬 𝐠𝐢𝐯𝐞𝐚𝐰𝐚𝐲 𝐜𝐚𝐮𝐬𝐞 𝐲𝐨𝐮𝐫 𝐧𝐨𝐭 𝐭𝐡𝐞 𝐡𝐨𝐬𝐭', { ephemeral: true });
		...
		if (button.id.startsWith('br')) {
			...
			if (button.clicker.member.roles.cache.has(role)) {
				...
				button.reply.send(`𝐈 𝐡𝐚𝐯𝐞 𝐫𝐞𝐦𝐨𝐯𝐞𝐝 𝐭𝐡𝐞 <@&${role}> 𝐫𝐨𝐥𝐞 𝐟𝐫𝐨𝐦 𝐲𝐨𝐮!`, true);
			}
			else {
				button.clicker.member.roles.add(role);
				button.reply.send(`𝐈 𝐡𝐚𝐯𝐞 𝐚𝐝𝐝𝐞𝐝 𝐭𝐡𝐞 <@&${role}> 𝐫𝐨𝐥𝐞 𝐭𝐨 𝐲𝐨𝐮!`, true);
			}
		}
	}
	static async dropclick(client, menu) {
		...
		if(menu.id == 'dr') {
			...
			if (menu.clicker.member.roles.cache.has(role)) {
				...
				menu.reply.send(`𝐈 𝐡𝐚𝐯𝐞 𝐫𝐞𝐦𝐨𝐯𝐞𝐝 𝐭𝐡𝐞 <@&${role}> 𝐫𝐨𝐥𝐞 𝐟𝐫𝐨𝐦 𝐲𝐨𝐮!`, true);
			}
			else {
				menu.clicker.member.roles.add(role);
				menu.reply.send(`𝐈 𝐡𝐚𝐯𝐞 𝐚𝐝𝐝𝐞𝐝 𝐭𝐡𝐞 <@&${role}> 𝐫𝐨𝐥𝐞 𝐭𝐨 𝐲𝐨𝐮!`, true);
			}
		}
	}
}
